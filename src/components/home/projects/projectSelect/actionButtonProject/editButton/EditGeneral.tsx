...
function Editgeneral(props: any){
    ...
    React.useEffect(()=>{
        ...
        if(props.watch("isAllUserBelongTo")){
            setCheck(props.watch("isAllUserBelongTo"));
        }
    });
    const customer = useAppSelector((state) => state.projects.customer);
    const handleCheckbox = (index: boolean) => {
        setCheck(index);
        props.setValue("isAllUserBelongTo", index);
    };
    //console.log(props.watch("isAllUserBelongTo"))
        ...
        <div>
            ...
            <Grid container sx={{marginTop: "20px"}}>
                ...
                <Grid item lg={10} md={12} sm={12} xs={12}>
                    {(props.errors.name)&& <span><p style={{color: "red"}}>This name is required</p></span>}
                    <TextField  
                        fullWidth
                        sx={{width: {lg:"66%" }}}
